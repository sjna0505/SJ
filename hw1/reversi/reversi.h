...
struct Square {
    ...
    bool operator==(const Square::SquareValue t);
};
...
private:
    ...
    char index_to_row(size_t ind) const {
        return (char)(ind + 'a' -1);
    }

    size_t char_to_index(char ind) const {
        return (size_t)(ind - '0');
    }

    void flipInsides(char row, size_t column, Square::SquareValue turn);
